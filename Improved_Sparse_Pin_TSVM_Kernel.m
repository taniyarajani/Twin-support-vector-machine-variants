...
function [accuracy, non_zero_dual_variables, training_time, lambda] = Improved_Sparse_Pin_TSVM_Kernel(X1_Train, X2_Train, X_Test, Y_Test, c, gamma, epsilon, tau,delta)
...
%Bin = [c.*e2; zeros(m2, 1)];
Aeq = [(-1/tau).*Im2 (1 + 1/tau).*Im2; Im2 -Im2];
Beq = [c.*e2; zeros(m2, 1)];
...
%Bin = [c.*e1; zeros(m1, 1)];
Aeq = [(-1/tau).*Im1 (1 + 1/tau).*Im1; Im1 -Im1];
Beq = [c.*e1; zeros(m1, 1)];
